...
import { isHeader, initTrackConfig, getFrameDuration ,appendFrame  } from './adts.js';
let ADTS = {
  isHeader:isHeader,
  initTrackConfig :initTrackConfig , 
...
class TSDemuxer {
  constructor (observer, remuxer, config, typeSupported) {
    ...
    this.remuxer = remuxer || new MP4Remuxer( observer );
    this.sampleAes = null;
  }
  ...
  static probe (data) {
    ...
    if (syncOffset < 0) {
      ...
      if (syncOffset) {
        window.debug && console.warn(`MPEG2-TS detected but first sync word found @ offset ${syncOffset}, junk ahead ?`);
      }
  ...
  append (data, timeOffset, contiguous, accurateTimeOffset) {
    ...
    for (start = syncOffset; start < len; start += 188) {
      if (data[start] === 0x47) {
        ...
        //this.observer.trigger(HLSEvent.ERROR, { type: ErrorTypes.MEDIA_ERROR, details: ErrorDetails.FRAG_PARSING_ERROR, fatal: false, reason: 'TS packet did not start with 0x47' });
      }
    }
