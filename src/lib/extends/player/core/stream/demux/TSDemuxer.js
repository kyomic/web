...
class TSDemuxer{
	constructor(observer){
        ...
        this.contiguous = false; //是否连续标记
        this.remuxer = new MP4Remuxer( this.observer )
	}
    ...
    push(data, audioCodec, videoCodec, timeOffset, cc, level, sn, duration) {
        var avcData, aacData, id3Data, start, len = data.length,
        stt, pid, atf, offset;
        ...
        if (cc !== this.lastCC) {
            window.debug && console.log('discontinuity detected');
            this.insertDiscontinuity();
            this.lastCC = cc;
        } else if (level !== this.lastLevel) {
            window.debug && console.log('level switch detected');
            this.switchLevel();
        ...
        for (start = 0; start < len; start += 188) {
            if (data[start] === 0x47) {
                stt = !!(data[start + 1] & 0x40); //将data与 1000000相与，等价于　(data[start+1] & 64) >> 6
                ...
                if (pmtParsed) {//stt = false;if(!avcData) avcData = {data:[],size:0};if(!aacData) aacData = {data:[],size:0};
                    if (pid === avcId) {
                        if (stt) {
                            if (avcData) {
    ...
    _parsePES(stream) {
        var i = 0,
        frag, pesFlags, pesPrefix, pesLen, pesHdrLen, pesData, pesPts, pesDts, payloadStartOffset;
...
as Bitwise operators treat their operands as a sequence of 32 bits */
    ...
    _parseAVCPES(pes) {
        ...
        if (debug || debugString.length) {
            window.debug && console.log(debugString);
        }
    ...
    _parseAACPES(pes) {
        ...
        aacSample, data = pes.data,
        config, adtsFrameSize, adtsStartOffset, adtsHeaderLen, stamp, nbSamples, len;
        if (this.aacOverFlow) {
        ...
        if (!track.audiosamplerate) {
            ...
            window.debug && console.log('parsed codec:' + track.codec + ',rate:' + config.samplerate + ',nb channel:' + config.channelCount);
        }
        nbSamples = 0;
        while (adtsStartOffset + 5 < len) {
            ...
            if (adtsFrameSize > 0 && adtsStartOffset + adtsHeaderLen + adtsFrameSize <= len) {
                aacSample = {
                    unit: data.subarray(adtsStartOffset + adtsHeaderLen, adtsStartOffset + adtsHeaderLen + adtsFrameSize),
                    pts: stamp,
                    dts: stamp
                };
    ...
    _ADTStoAudioConfig(data, offset, audioCodec) {
        ...
        window.debug && console.log('manifest codec:' + audioCodec + ',ADTS data:type:' + adtsObjectType + ',sampleingIndex:' + adtsSampleingIndex + '[' + adtsSampleingRates[adtsSampleingIndex] + 'Hz],channelConfig:' + adtsChanelConfig);
