let {MP4} = require('./mp4-generator');
let { HLSEvent } = require("../../../event");
let { ErrorTypes, ErrorCodes } = require('../../../const/Errors');
class MP4Remuxer{
    constructor( observer ){
        this.observer = observer;
        this.ISGenerated = false;
        this.PES2MP4SCALEFACTOR = 4;
        //这个90000是指1秒的tick数，其实也就是把1秒中分为90000份
        this.PES_TIMESCALE = 90000;

        this.MP4_TIMESCALE = this.PES_TIMESCALE / this.PES2MP4SCALEFACTOR;//22500
        //兼容
        this.timescale = this.MP4_TIMESCALE;
    }
    ...
    generateIS(audioTrack, videoTrack, timeOffset) {
        ...
        if (nbAudio === 0 && nbVideo === 0) {
            observer.trigger( PlayerEvent.ERROR, {
                code: ErrorTypes.MEDIA_ERROR,
                details: ErrorCodes.FRAG_PARSING_ERROR,
                fatal: false,
            ...
            if (audioTrack.config) {
                observer.trigger( HLSEvent.FRAG_PARSING_INIT_SEGMENT, {
     ...
     * @param {boolean} contiguous 是否相邻的
     */
    remuxVideo(track, timeOffset, contiguous) {
        var view, i = 8,
        pesTimeScale = this.PES_TIMESCALE,
        pes2mp4ScaleFactor = this.PES2MP4SCALEFACTOR,
        avcSample, mp4Sample, mp4SampleLength, unit, mdat, moof, firstPTS, firstDTS, lastDTS, pts, dts, ptsnorm, dtsnorm, samples = [];
...
(need 8 more bytes to fill length and mpdat type) */
            ...
            if (lastDTS !== undefined) {
                ...
                if (contiguous || Math.abs(delta) < 600) {
                    if (delta) {
                        ...
                        window.debug && console.log('Video/PTS/DTS adjusted:' + ptsnorm + '/' + dtsnorm);
                    }
                }
        ...
        this.observer.trigger( HLSEvent.FRAG_PARSING_DATA, {
            moof: moof,
            mdat: mdat,
    ...
    remuxAudio(track, timeOffset, contiguous) {
        ...
        pesTimeScale = this.PES_TIMESCALE,
        pes2mp4ScaleFactor = this.PES2MP4SCALEFACTOR,
        aacSample, mp4Sample, unit, mdat, moof, firstPTS, firstDTS, lastDTS, pts, dts, ptsnorm, dtsnorm, samples = [];
...
(need 8 more bytes to fill length and mdat type) */
        ...
        while (track.samples.length) {
            ...
            if (lastDTS !== undefined) {
                ...
                mp4Sample.duration = (dtsnorm - lastDTS) / pes2mp4ScaleFactor;
                if (mp4Sample.duration < 0) {
                    window.debug && console.log('invalid AAC sample duration at PTS:' + aacSample.pts + ':' + mp4Sample.duration);
                    mp4Sample.duration = 0;
                }
            } else {
                ...
                if (contiguous || Math.abs(delta) < 600) {
                    ...
                    if (delta) {
                        if (delta > 1) {
                            ...
                            window.debug && console.log( - delta + ' ms overlapping between AAC samples detected');
                        }
                        // set DTS to next DTS
