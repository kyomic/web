...
class MP4Remuxer {
  ...
  generateIS( audioTrack, videoTrack, timeOffset ){
    ...
    if (audioTrack.config ) {
      ...
      audioTrack.timescale = audioTrack.inputTimeScale;
  ...
  remuxVideo(track, timeOffset, contiguous, audioTrackLength, accurateTimeOffset) {    
    ...
    if ( this._videoStashedLastSample ) {
        var lastStashedSample = this._videoStashedLastSample;
        this._videoStashedLastSample = null;
        inputSamples.unshift(lastStashedSample);
        mdatSize += lastStashedSample.length;
    }
    if (lastSample != null) {
    ...
    */
    ...
    for (let j = 0, nbSamples = inputSamples.length; j < nbSamples; j++) {
      ...
      if (lastPTS !== undefined ) {
        ...
        mp4Sample.duration = Math.round((pts - lastPTS) );
      } else {
