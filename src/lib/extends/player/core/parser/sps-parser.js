...
import ExpGolomb from '../ExpGolomb.js';
...
class SPSParser {
    ...
    static parseSPS(uint8array) {
        ...
        if (pic_order_cnt_type === 0) {
            ...
            gb.readEG();  // offset_for_non_ref_pic
            gb.readEG();  // offset_for_top_to_bottom_field
            let num_ref_frames_in_pic_order_cnt_cycle = gb.readUEG();
            for (let i = 0; i < num_ref_frames_in_pic_order_cnt_cycle; i++) {
                gb.readEG();  // offset_for_ref_frame
            }
        }
        ...
        let frame_cropping_flag = gb.readBoolean();
